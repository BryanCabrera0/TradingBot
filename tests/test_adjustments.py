...
class AdjustmentEngineTests(unittest.TestCase):
    def test_disabled_engine_returns_none_plan(self) -> None:
        ...
        plan = engine.evaluate(
            position=_position(), regime="normal", iv_change_since_entry=0.0
        )
    ...
    def test_add_wing_when_high_vol_and_tested(self) -> None:
        ...
        plan = engine.evaluate(
            position=_position(), regime="HIGH_VOL_CHOP", iv_change_since_entry=0.20
        )
        self.assertEqual(plan.action, "add_wing")

    def test_roll_tested_side_when_losing(self) -> None:
        engine = AdjustmentEngine(
            {"enabled": True, "min_dte_remaining": 7, "max_adjustments_per_position": 2}
        )
        plan = engine.evaluate(
            position=_position(current_value=1.4),
            regime="normal",
            iv_change_since_entry=0.0,
        )
    ...
    def test_max_adjustments_block(self) -> None:
        ...
        plan = engine.evaluate(
            position=_position(details={"short_strike": 100.5, "adjustment_count": 1}),
            regime="normal",
            iv_change_since_entry=0.0,
        )
        self.assertEqual(plan.action, "none")
