...
class StrategyContextFilterTests(unittest.TestCase):
    ...
    def test_iron_condor_blocks_high_vol_of_vol(self) -> None:
        strategy = IronCondorStrategy(
            {"min_dte": 20, "max_dte": 45, "short_delta": 0.16, "spread_width": 5}
        )
        signals = strategy.scan_for_entries(
            "SPY",
            _spread_chain(),
    ...
    def test_calendar_spread_requires_backwardation_in_vol_surface_context(
        self,
    ) -> None:
    ...
    def test_broken_wing_flow_bias_can_override_regime_direction(self) -> None:
        strategy = BrokenWingButterflyStrategy(
            {"min_dte": 20, "max_dte": 45, "short_delta": 0.30}
        )
        signals = strategy.scan_for_entries(
            "SPY",
            _bwb_chain(),
    ...
    def test_credit_spreads_limits_signals_per_symbol(self) -> None:
        strategy = CreditSpreadStrategy(
            {"direction": "bull_put", "min_credit_pct": 0.1}
        )
        strategy.iv_history = mock.Mock(update_and_rank=mock.Mock(return_value=50.0))
        base = SpreadAnalysis(
            symbol="SPY",
        ...
        signal_a = TradeSignal(
            action="open", strategy="bull_put_spread", symbol="SPY", analysis=base
        )
        ...
        strategy._scan_put_spreads = mock.Mock(
            return_value=[signal_a, signal_b, signal_c]
        )
