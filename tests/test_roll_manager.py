...
class RollManagerTests(unittest.TestCase):
    def test_profit_roll_trigger(self) -> None:
        manager = RollManager({"enabled": True, "min_dte_trigger": 7, "min_credit_for_roll": 0.2, "max_rolls_per_position": 2})
        position = {"dte_remaining": 6, "entry_credit": 1.0, "current_value": 0.5, "details": {"roll_count": 0}}
    ...
    def test_defensive_roll_trigger(self) -> None:
        manager = RollManager({"enabled": True, "min_dte_trigger": 7, "min_credit_for_roll": 0.1, "max_rolls_per_position": 2})
        position = {"dte_remaining": 9, "entry_credit": 1.0, "current_value": 1.5, "details": {"roll_count": 0}}
    ...
    def test_max_rolls_blocks_roll(self) -> None:
        ...
        position = {"dte_remaining": 5, "entry_credit": 1.0, "current_value": 0.8, "details": {"roll_count": 1}}
        decision = manager.evaluate(position, regime="normal")
        self.assertFalse(decision.should_roll)

    def test_disabled_returns_false(self) -> None:
        manager = RollManager({"enabled": False})
        decision = manager.evaluate({"dte_remaining": 5, "entry_credit": 1.0, "current_value": 0.7, "details": {}}, regime="normal")
        self.assertFalse(decision.should_roll)
