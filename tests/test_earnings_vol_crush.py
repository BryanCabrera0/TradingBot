...
def _chain() -> dict:
    return {
        "calls": {
            "2026-03-05": [
                {"dte": 2, "strike": 105.0, "bid": 1.0, "ask": 1.2, "mid": 1.1, "delta": 0.12, "gamma": 0.01, "theta": -0.04, "vega": 0.06, "iv": 58.0, "volume": 500, "open_interest": 800},
                {"dte": 2, "strike": 115.0, "bid": 0.2, "ask": 0.3, "mid": 0.25, "delta": 0.03, "gamma": 0.005, "theta": -0.01, "vega": 0.03, "iv": 55.0, "volume": 300, "open_interest": 500},
            ]
        },
        "puts": {
            "2026-03-05": [
                {"dte": 2, "strike": 95.0, "bid": 1.0, "ask": 1.2, "mid": 1.1, "delta": -0.12, "gamma": 0.01, "theta": -0.04, "vega": 0.06, "iv": 59.0, "volume": 500, "open_interest": 800},
                {"dte": 2, "strike": 85.0, "bid": 0.2, "ask": 0.3, "mid": 0.25, "delta": -0.03, "gamma": 0.005, "theta": -0.01, "vega": 0.03, "iv": 56.0, "volume": 300, "open_interest": 500},
            ]
        },
    }
...
class EarningsVolCrushStrategyTests(unittest.TestCase):
    def test_requires_iv_rank(self) -> None:
        ...
        signals = strategy.scan_for_entries("TSLA", _chain(), 100.0, market_context={"iv_rank": 50.0, "max_expiration": "2026-03-05"})
        self.assertEqual(signals, [])

    def test_requires_earnings_window(self) -> None:
        strategy = EarningsVolCrushStrategy({"min_iv_rank": 70.0})
        strategy.earnings_calendar = mock.Mock(earnings_within_window=mock.Mock(return_value=(False, None)))
        signals = strategy.scan_for_entries("TSLA", _chain(), 100.0, market_context={"iv_rank": 80.0, "max_expiration": "2026-03-05"})
        self.assertEqual(signals, [])
    ...
    def test_exit_on_profit(self) -> None:
        ...
        positions = [{"position_id": "e1", "strategy": "earnings_vol_crush", "symbol": "TSLA", "status": "open", "entry_credit": 1.0, "current_value": 0.6, "quantity": 1, "dte_remaining": 1}]
        signals = strategy.check_exits(positions, market_client=None)
        self.assertEqual(len(signals), 1)

    def test_exit_on_event_complete(self) -> None:
        strategy = EarningsVolCrushStrategy({})
        positions = [{"position_id": "e2", "strategy": "earnings_vol_crush", "symbol": "TSLA", "status": "open", "entry_credit": 1.0, "current_value": 0.95, "quantity": 1, "dte_remaining": 0}]
        signals = strategy.check_exits(positions, market_client=None)
        self.assertEqual(len(signals), 1)
