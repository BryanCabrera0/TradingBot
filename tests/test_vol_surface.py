...
class VolSurfaceTests(unittest.TestCase):
    def test_analyzer_flags_positive_vol_risk_premium(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            price_history = [
                {"close": v}
                for v in [100, 101, 99, 102, 100, 101, 100, 99, 101, 100, 102, 101, 100]
            ]

            context = analyzer.analyze(
                symbol="SPY", chain_data=_chain(32.0, 25.0), price_history=price_history
            )
    ...
    def test_skew_regime_detects_put_skew(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            context = analyzer.analyze(
                symbol="SPY", chain_data=_chain(35.0, 28.0), price_history=[]
            )
    ...
    def test_term_structure_flat_when_insufficient_points(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            analyzer = VolSurfaceAnalyzer(history)
            chain_data = {
                "calls": {"2026-03-20": [{"dte": 25, "iv": 22.0, "delta": 0.2}]},
                "puts": {},
            }

            context = analyzer.analyze(
                symbol="QQQ", chain_data=chain_data, price_history=[]
            )
    ...
    def test_iv_rank_and_percentile_are_bounded(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            analyzer = VolSurfaceAnalyzer(history)
            context = analyzer.analyze(
                symbol="AAPL", chain_data=_chain(40.0, 30.0), price_history=[]
            )
    ...
    def test_term_structure_threshold_boundaries(self) -> None:
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            backward = analyzer.analyze(
                symbol="SPY", chain_data=_chain(36.0, 33.0), price_history=[]
            )
            contango = analyzer.analyze(
                symbol="SPY", chain_data=_chain(28.0, 31.0), price_history=[]
            )
            flat = analyzer.analyze(
                symbol="SPY", chain_data=_chain(34.0, 32.0), price_history=[]
            )
