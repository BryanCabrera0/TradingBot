...
class EconomicCalendar:
    ...
    def adjust_signal(
        self, signal, *, as_of: Optional[date] = None
    ) -> tuple[bool, str]:
        ...
        if action == "reduce_size":
            signal.size_multiplier = max(
                0.1, float(signal.size_multiplier or 1.0) * 0.70
            )
        ...
        if action == "widen" and analysis is not None:
            ...
            if widened:
                signal.size_multiplier = max(
                    0.1, float(signal.size_multiplier or 1.0) * 0.85
                )
    ...
    def _parse_events(rows: list[dict]) -> list[EconEvent]:
        ...
        for row in rows or []:
            ...
            if not name:
                continue
            if (
                name not in HIGH_IMPACT
                and str(row.get("impact", "")).lower() != "macro"
            ):
                continue
            events.append(
                EconEvent(
